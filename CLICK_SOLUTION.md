...
> 当前状态：文件级通行证弹窗坐标缓存仍用于定位，但旧的“点击按钮 + 固定等待 + 输入 + 确认”合并快路径已禁用。通行证按钮点击后必须先检测弹窗出现，失败时清空缓存并重新模板匹配。后续修改点击、输入、停止清理前必须按 `D:\Ai\skills\launcher-regression-guard\SKILL.md` 做防回归检查。

> 2026-06-09 备注：本次只修串行按钮语义和 run_plan 窗口预检，未修改 Dm 前台点击、模板匹配、剪贴板粘贴、弹窗坐标缓存。禁止回退到 Playwright/CDP/PostMessage/SendMessage 点击 canvas 或 Dm BindWindow。
...
一次子进程调用完成点击输入框+输入文本，节省 ~1s。通行证按钮本身不能再和输入+确认合并成“固定等待快路径”；必须先点击按钮并确认通行证登录弹窗出现后，才允许执行输入+确认 chain。`creationflags=CREATE_NO_WINDOW` 防止弹黑框。
...
- 能读取通行证弹窗坐标缓存。
- 旧合并快路径已禁用，缓存坐标不会绕过弹窗校验。
- 输入+确认仍可在弹窗确认出现后使用 Dm chain。
- 9 个单层账号批量快速登录 + 统一校验最终全部成功。

安全边界：

- Dm chain 只允许用于弹窗已出现后的输入/确认。
- 禁止恢复“点击通行证按钮后固定等待并直接输入”的快路径。
- 不修改通行证复制优先逻辑。
- 不修改 OCR 兜底规则。
- 不修改登录成功校验规则。
