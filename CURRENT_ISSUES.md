...
- 方式一通行证上号已验证通过。
- 后台方式一单账号 live 已验证成功。
- 后台当前层串行已完成 `存钻` 前 2 个账号小范围 live 验证并成功：第 1 个账号首次成功，复跑已进入游戏跳过；第 2 个账号修复后成功。
- 后台全部串行已接入，仍需更大范围 live 验证。
- 后台并发未接入，并发固定为 1；后台成功窗口默认保留。
- 方式二账号密码 + 通行证上号已验证通过。
- 源码模式和 exe 模式均已测试通过。
- 固定参数排列和行数列数排列均已支持。
- 单层账号和四层账号已隔离。
...
1. 后台截图登录窗口。
2. 判断是否已进入游戏，已进入则跳过 OCR。
3. 二维码登录页直接走 `red_bar_box` 局部 OCR 多证据提取。
4. OCR 成功后继续后台方式一流程。
5. OCR 失败则保存失败现场并报错：`后台 OCR 未能可靠识别本次通行证`。

后台 WM_GETTEXT、UIA、后台 Ctrl+C、剪贴板 marker、剪贴板读取和剪贴板恢复链路已在真实验证中失败，结论为没有实际价值，后台模式已废弃这些复制/读取路径。后台方式一单账号 live 已验证成功；后台当前层串行 `存钻` 前 2 个账号小范围 live 验证成功；后台全部串行已接入但等待更大范围验证；黑屏保护暂不实现。

后台串行失败策略：单个账号失败不会中断整轮，会记录失败并继续下一个；用户点击停止、依赖预检失败、读取设置失败或运行前预检失败才会中断整个串行。后台流程不调用 `SetForegroundWindow`，不使用全局鼠标/键盘。

前台 OCR 不再作为第一优先级。原因是 OCR 存在单字符误识别风险，例如真实值 `4425cbaa` 可能被识别为 `4425ebaa`，错误位置为 `c` 被识别为 `e`。
...
- `层级=全部` 只显示 `include_in_all=true` 的分组账号。
- `全部串行` 只执行 `include_in_all=true` 的分组，并且数据来源必须与 `层级=全部` 的账号列表一致。
...
- `全部串行`：只运行“全部串行分组设置”中 `include_in_all=true` 的分组；当层级为 `全部` 时，它与当前表格显示范围一致。
...
| OCR 通行证识别 | 部分截图 OCR 质量不稳定 | 前台复制优先、OCR 兜底；后台使用 red_bar_box 局部 OCR 多证据，低置信度必须失败 |
...
- 前台复制优先已经降低 OCR 风险；后台使用已 live 验证的 red_bar_box 局部 OCR 强证据路径
