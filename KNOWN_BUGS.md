...
## 14. 单窗口补位后普通排列不能再全局重排

**场景**：31 个窗口已经排列和编号完成，某个窗口卡死后通过槽位补位，例如 `slot 11` 只启动 1 个新窗口并移动回原位置。

**问题**：如果普通“排列窗口”仍然调用全局 `list_game_windows() → sort_game_windows() → tile_game_windows()`，就可能按当前 `hwnd`、枚举顺序、坐标或标题重新排序，导致槽位映射失效。

**正确做法**：

1. 存在有效 `window_slots.json` 时，普通“排列窗口”默认调用槽位恢复逻辑。
2. 槽位恢复只读取保存的 `slot_no`、位置、尺寸、标题和当前 `hwnd` 绑定关系。
3. 不按 `hwnd` / 枚举顺序重新排序。
4. 不覆盖 `window_slots.json`。
5. 普通“重命名”只按槽位标题修正，不全局重新编号。
6. 只有用户明确点击“重新生成槽位”并二次确认后，才允许全局排列、全局编号并覆盖 `window_slots.json`。

**出现次数**：1 次（slot 11 补位成功后识别出的风险）

## 15. 槽位文件不完整时修复窗口不能直接失败

**场景**：已有窗口已排列并编号，但 `window_slots.json` 中缺少某个目标槽位，例如 `slot 15`。用户只想修复该槽位，不想重新排列全部窗口。

**问题**：如果 `repair_window_slot(slot_no)` 只依赖 `window_slots.json`，缺少目标 slot 时会直接报“未找到历史位置”，无法完成单窗口补位。

**正确做法**：

1. 优先读取 `window_slots.json` 中的目标 slot。
2. 如果缺失，枚举当前窗口并从标题编号补齐该 slot 的当前位置。
3. 如果目标窗口已关闭，按固定排列参数推导位置：`row = (slot_no - 1) // per_row`，`col = (slot_no - 1) % per_row`。
4. 只启动 1 个新窗口并移动到目标 slot。
5. 不重新排列全部窗口，不重新编号全部窗口，不修改其它窗口标题。
6. “刷新槽位映射”只能保存当前窗口位置，不允许移动或重命名窗口。

**出现次数**：1 次（slot 15 历史位置缺失时发现）

## 16. 全部窗口重新启动不能沿用旧 hwnd 槽位快照

**场景**：电脑重启或用户关闭全部 H5 窗口后，再通过窗口管理区批量启动 31 个窗口。此时旧 `window_slots.json` 中的 `hwnd` 已经全部失效，但长期排列参数仍保存在 `window_manager_settings.json`。

**问题**：如果批量启动后仍然优先按旧 `window_slots.json` 恢复，可能把新会话错误绑定到旧 `hwnd` 快照，导致新排列参数不生效，甚至让用户误以为“排列窗口”又按枚举顺序乱序。

**正确做法**：

1. 启动前识别到 H5 窗口数为 0 时，判定为“新窗口会话”。
2. 新窗口会话批量启动后，必须按当前 `window_manager_settings.json` 的排列参数重新排列和重命名。
3. 重新保存新的 `window_slots.json`，并写入 `environment`、DPI、缩放比例、屏幕分辨率和 `layout_params`。
4. 普通“排列窗口”在后续有有效槽位快照时仍按槽位恢复，不覆盖槽位文件。
5. 不允许写死坐标、分辨率、缩放比例、窗口数量、路径或 slot 编号。

**现场验证**：2026-06-07 已验证。先通过窗口管理层关闭当前 31 个 H5 窗口并确认窗口数降为 0，再批量启动 31 个窗口；排列/重命名成功并生成新槽位；随后普通按槽位恢复 31/31 成功且未覆盖槽位文件。

**出现次数**：1 次（区分单窗口修复、普通排列和全部窗口重新启动时发现）
